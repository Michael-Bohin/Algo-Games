let cities: { [key: string]: [number, number] } = {
    Reykjavik:  [67, 135],
    Dublin:     [381, 356],
    London:     [516, 391],
    Amsterdam:  [602, 371],
...
function randint(min: number, max: number): number { // inclusive type of both min and max
    return Math.floor(Math.random() * (max - min + 1) ) + min;
}

function rand_permutation(list: Array<string>, start: string, goal: string): Array<string> {
    for(var i = 0; i < list.length; i++) {
        let choose: number = randint(i, list.length-1);
        var temp = list[i];
        list[i] = list[choose];
        list[choose] = temp;
        if(list[i] == start) {
...
let perm_cities: Array<string> = ['hi', 'bye']; // initialising permutated cities list outside of onLoad function, in order to be able to acess it from other function too
...
let edges: string[][] = [['hi', 'bye']];
let start: string = '';
let goal: string = '';
let step: number = 1; // main indicator for flight control panel, moreover check result only lets you further if step == 4
...
function add(i: number): void {
    if(step == 4 || (step != 1 && i == 3)) {
        ...
        // 1:
        let adding: string = document.getElementById(`btn_add${i}`).innerHTML;
        ...
        let city1 : string = "";
        let city2 : string = "";
        edges.forEach(function(item: string[]) {
            let from: string = item[0];
            let to: string = item[1];
            if(from == adding) {
...
function remove_last(): void {
    if(step == 1) {
        ...
        if(step == 4) {
            ...
            let pred: string = modifying.innerHTML;
            modifying = document.getElementById("btn_remove");
            ...
            edges.forEach(function(edge) {
                let from: string = edge[0];
                let to: string = edge[1];
                if(from == pred) {
                    if(add1 == "") {
                        add1 = to;
                    } else {
        ...
        if(step == 3) {
            ...
            let pred: string = document.getElementById("path2").innerHTML;
        ...
        if(step == 2) {
            ...
            let pred: string = document.getElementById("path1").innerHTML;
            modify.innerHTML = pred;
...
function check(): void {
    if(step != 4) {
        ...
        let first_city: string = document.getElementById('path2').innerHTML;
        let second_city: string = document.getElementById('path3').innerHTML;
        let third_city: string = document.getElementById('path4').innerHTML;
        ...
        for(let i: number = 1; i < 13; i++) {
            ...
            for(let j: number = 0; j < 4; j++) {
                let from: string = edges_temp[j][0];
                let to: string = edges_temp[j][1];
                let [fx , fy]: [number, number] = cities[from], [tx, ty]: [number, number] = cities[to];
...
function onLoad() {
    ...
    let paths: string = sessionStorage.getItem('paths');
    let edge_type:string = sessionStorage.getItem('edge_type');
    console.log(start);
    console.log(goal);
    console.log(paths);
    ...
    edges = [
        ...
        [perm_cities[0], perm_cities[2]], [perm_cities[0], perm_cities[3]], [perm_cities[0], perm_cities[4]], // 0, 1, 2

        // step 2, edges from first cities to second cities
        [perm_cities[2], perm_cities[5]], [perm_cities[2], perm_cities[6]], // 3, 4
        [perm_cities[3], perm_cities[7]], [perm_cities[3], perm_cities[8]], // 5, 6
        [perm_cities[4], perm_cities[9]], [perm_cities[4], perm_cities[10]], // 7, 8

        //step 3, edge from second cities to third cities 
        [perm_cities[5], perm_cities[11]], [perm_cities[5], perm_cities[12]], // 9, 10
        [perm_cities[6], perm_cities[13]], [perm_cities[6], perm_cities[14]], // 11, 12
        [perm_cities[7], perm_cities[15]], [perm_cities[7], perm_cities[16]], // 13, 14
        [perm_cities[8], perm_cities[17]], [perm_cities[8], perm_cities[18]], // 15, 16
        [perm_cities[9], perm_cities[19]], [perm_cities[9], perm_cities[20]],  // 17, 18
        [perm_cities[10], perm_cities[21]], [perm_cities[10], perm_cities[22]], // 19, 20

        // step 4, edge from third cities to goal 
        [perm_cities[11], perm_cities[1]],  // 21
        [perm_cities[12], perm_cities[1]],  // 22
        [perm_cities[13], perm_cities[1]],  // 23
        [perm_cities[14], perm_cities[1]],  // 24
        [perm_cities[15], perm_cities[1]],  // 25
        [perm_cities[16], perm_cities[1]],  // 26 
        [perm_cities[17], perm_cities[1]],  // 27 
        [perm_cities[18], perm_cities[1]],  // 28 
        [perm_cities[19], perm_cities[1]],  // 29 
        [perm_cities[20], perm_cities[1]],  // 30
        [perm_cities[21], perm_cities[1]],  // 31 
        [perm_cities[22], perm_cities[1]],  // 32
    ];
...
function hide_all(): void {
    cx.clearRect(0, 0, canvas.width, canvas.height);
}

function display_all(): void {
    cx.clearRect(0, 0, canvas.width, canvas.height);
    cx.strokeStyle = '#0f0';
    cx.fillStyle = '#0f0';
    cx.font = 'bold 24px sans-serif';
...
function visualise(i: number): void {
    cx.clearRect(0, 0, canvas.width, canvas.height);
    cx.strokeStyle = '#0f0';
    cx.fillStyle = '#0f0';
    cx.font = 'bold 24px sans-serif';
