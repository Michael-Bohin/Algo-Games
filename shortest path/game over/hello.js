...
for (let i = 1; i < 13; i++) {
    ...
    let string_array = calculation.split(" ");
    let calc_with_plus = string_array[1] + " + " + string_array[2] + " + " + string_array[3] + " + " + string_array[4] + " =";
...
for (let i = 1; i < 13; i++) {
    for (let j = 1; j < 12; j++) {
        let num1 = parseInt(document.getElementById(`res${j}`).innerHTML);
        let num2 = parseInt(document.getElementById(`res${j + 1}`).innerHTML);
        if (num1 > num2) {
            // result on lower row is longer number => swap rows
            let temp_path = document.getElementById(`path${j}`).innerHTML;
            let temp_calc = document.getElementById(`calc${j}`).innerHTML;
            let temp_res = document.getElementById(`res${j}`).innerHTML;
...
for (let i = 1; i < 13; i++) {
    let path = document.getElementById(`path${i}`).innerHTML;
