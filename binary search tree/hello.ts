...
class BST_i {
    ...
    public insert(i: number): void {
        let temp: Node_i = this.root;
        let previous: Node_i = null;
        while(temp != null) {
...
class BST_s {
    ...
    public insert(i: string): void {
        let temp: Node_s = this.root;
        let previous: Node_s = null;
        while(temp != null) {
            // search for the Node that we would to insert the value to
            previous = temp;
            if(temp.val > i) {
    ...
    // return true if val of temp == i and once temp is null return false
    // and no need to store previous
    public contains(i: string): boolean {
        let temp: Node_s = this.root;
        while(temp != null) {
            // search for the Node that we would to insert the value to
            if(temp.val > i) {
...
let list_int: number[] = new Array(101);
for(let i: number = 0; i < list_int.length; i++) {
    list_int[i] = i;
}

let universe: string = "";
let x: BST_i = new BST_i();
let y: BST_s = new BST_s();
let question_int: number = -1;
let question_string: string = "rampam";
var current_iNode: Node_i;
var current_sNode: Node_s;
let seen_iArray: number[] = new Array(20);
let seen_sArray: string[] = new Array(20);
let seen_array_index: number = 0;
let parent_card_revealed: boolean = false;
let seen_cards_count: number = 0;
let canvas = document.querySelector('canvas');
let cx = canvas.getContext('2d');
cx.beginPath();
var moving_BST_animation_in_progress: boolean = false;
...
function random_int_permutation(): void { // did want this function to return the permutated array, but didn't find syntax with what compiler would have been happy
    for(let i: number = 0; i < list_int.length; i++) {
        let choose: number = randint(i, list_int.length-1);
        let temp: number = list_int[i];
...
function random_string_permutation(): void { // did want this function to return the permutated array, but didn't find syntax with what compiler would have been happy
    for(let i: number = 0; i < list_str.length; i++) {
        let choose: number = randint(i, list_str.length-1);
        let temp: string = list_str[i];
...
function integers(): void {
    universe = "int";
    random_int_permutation();
    list_int.forEach(element => {
        console.log(element);
    });
    for(let i: number = 0; i < list_int.length/2 ; i++) { // insert only first half of elements => each has 50% chance of being included
        x.insert(list_int[i]);
    }
...
function left_child(): void {
    ...
    modify.style.display = "none";
    setTimeout(function() {  // this is superawkward. compiler is happy like this, however when I write the function outside and call its name compiler is not happy. someone should probably fix this
